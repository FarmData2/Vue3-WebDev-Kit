...
5. Create and switch to a new feature branch for the assignment.
   - Running the commands below in the terminal in your codespace are one way to do this. You'll need to change the `<branch-name>` placeholder to be descriptive of the assignment that you are working on (e.g. `assignment-01`).
     ...
     ```
6. Complete **a little bit of work** on the assignment.
...
7. Stage and commit your changes to your feature branch.
   - Running the commands below in the terminal are one way to do this.
     ```bash
     git stage .
     git git commit -m "<descriptive commit message>"
     ```
8. Ensure that the commit was successful.
   - When making a commit, a number fo checks are performed on the changes being committed. These checks ensure that the changes are properly formatted, use good style, and do not contain broken links or spelling errors.
...
9. Push your feature branch to your fork (i.e. your `origin`).
   - Running the commands below in the terminal are one way to do this.
     ```bash
     git push origin <branch-name>
     ```
10. [Create a pull request](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request#creating-the-pull-request) for your feature branch, **if you have not already created one**.
...
11. Repeat steps 6-9, making, committing and pushing additional changes until you have completed the assignment.
12. [Request a review by CoPilot](https://docs.github.com/en/copilot/how-tos/use-copilot-agents/request-a-code-review/use-code-review#using-copilot-code-review).
13. Address all of the comments that CoPilot made during its pull request review by:
